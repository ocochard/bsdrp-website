...
---
This lab shows an interesting feature of strongSwan: an IPsec tunnel between devices that are both behind NAT gateways.
...
Enable debug mode for IKEv2, logging to the file `/var/log/charon.log`.
...
Digging further, we find that strongSwan generates IPsec packets using all the addresses configured on R2:
